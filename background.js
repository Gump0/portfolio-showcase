// background.js - credit to ngr99
// https://github.com/ngr900/constellation
// code has been slightly modified to fit my aesthetic

class Star {
  constructor(constellation) {
    this.x = random(
      0 + constellation.starRadius,
      constellation.canvas.width - constellation.starRadius,
    );
    this.y = random(
      0 + constellation.starRadius,
      constellation.canvas.height - constellation.starRadius,
    );
    this.r =
      constellation.starRadius *
      random(
        1 - constellation.starRadiusJitter,
        1 + constellation.starRadiusJitter,
      );
    this.context = constellation.context;
    this.hue =
      constellation.foregroundHue +
      random(1, 360) * constellation.foregroundHueJitter * plusOrMinus();
    this.saturation =
      constellation.foregroundSaturation *
      random(
        1 - constellation.foregroundSaturationJitter,
        1 + constellation.foregroundSaturationJitter,
      );
    this.lightness =
      constellation.foregroundLightness *
      random(
        1 - constellation.foregroundLightnessJitter,
        1 + constellation.foregroundLightnessJitter,
      );
    this.opacity = 100;
    this.constellation = constellation;
    this.vx =
      constellation.starVelocity *
      random(
        1 - constellation.starVelocityJitter,
        1 + constellation.starVelocityJitter,
      ) *
      plusOrMinus();
    this.vy =
  ...
  connections() {
    ...
    for (var i = 0; i < this.constellation.stars.length; i++) {
      if (
        ...
        Math.abs(this.x - this.constellation.stars[i].x) <
          this.constellation.connectionRadius &&
        Math.abs(this.y - this.constellation.stars[i].y) <
          this.constellation.connectionRadius
      ) {
    ...
    for (var i = 0; i < neighbors.length; i++) {
      ...
      if (
        Math.abs(this.x - this.constellation.mx) <
          this.constellation.revealRadius &&
        Math.abs(this.y - this.constellation.my) <
          this.constellation.revealRadius
      ) {
        t +=
          1 -
  ...
  move() {
    if (this.x <= 0 + this.r || this.x >= this.constellation.canvas.width)
      this.vx = -this.vx;
    if (this.y <= 0 + this.r || this.y >= this.constellation.canvas.height)
      this.vy = -this.vy;
    this.x += this.vx;
    this.y += this.vy;
  }
...
class Constellation {
  constructor(
    ...
    this.e.innerHTML =
      '<canvas id="constellation-canvas-' + constellationCount + '"></canvas>';
    this.canvas = document.getElementById(
      "constellation-canvas-" + constellationCount,
    );
  ...
  resize() {
    if (this.responsive) {
      ...
      this.numberOfStars =
        (this.starDensity * (this.e.offsetWidth * this.e.offsetHeight)) / 5000;
      this.fill();
    }
  }
...
function constellationHue(elementID, hue) {
  ...
  var preset = {
    name: "hue",
    properties: [
      defaultPreset.properties[0],
      defaultPreset.properties[1],
      defaultPreset.properties[2],
      defaultPreset.properties[3],
      defaultPreset.properties[4],
      defaultPreset.properties[5],
      defaultPreset.properties[6],
      0.05,
      defaultPreset.properties[8],
      hue,
      100,
      2,
      hue,
...
window.addEventListener(
  ...
  function (evt) {
    for (var i = 0; i < instances.length; i++) {
      if (instances[i].mouseInteractions) instances[i].mouse(evt);
    }
  },
  false,
);
...
window.addEventListener(
  ...
  function () {
    for (var i = 0; i < instances.length; i++) {
      if (instances[i].responsive) instances[i].resize();
    }
  },
  false,
);
